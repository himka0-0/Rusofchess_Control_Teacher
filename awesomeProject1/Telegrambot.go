...
func Validation_hash(text string) bool {
	var hashs []Table_telegram_bot
	err := db.Model(&Table_telegram_bot{}).Select("User_id,hash").Scan(&hashs).Error
	if err != nil {
...
func SaveUsers(username string, messageText string, chatID int64) {
	err := db.Model(&Table_telegram_bot{}).Where("hash=?", messageText).Updates(&Table_telegram_bot{First_name: username, Telegram_id: chatID, Vhod: true})
	if err != nil {
...
func messageBot(message string, nameStudent string, IdTeacher uint) {
	var onoff bool
	err := db.Model(&Table_telegram_bot{}).Select("vhod").Where("User_id=?", IdTeacher).Find(&onoff).Error
	if err != nil {
		log.Println("Проблема с вытаскиванием включателя уведомлений", err)
	}
	if onoff == true {
		var dialogue int64
		err = db.Model(&Table_telegram_bot{}).Select("telegram_id").Where("User_id=?", IdTeacher).Find(&dialogue).Error
		if err != nil {
			log.Println("Проблема с вытаскиванием включателя уведомлений", err)
		}
